...
function isoDate(value, filePath, field) {
  ...
  if (Number.isNaN(parsed.valueOf())) {
    throw new Error(
      `Invalid blog ${field} in "${filePath}": expected a valid date.`,
    );
  }
...
export default function blogSitemap({ site, routes }) {
  ...
  return {
      ...
      hooks: {
        "astro:config:setup": ({ config }) => {
          ...
          for (const pair of Object.values(routes))
            addAlternates(pair.es, pair.en);

          const blogDirectory = fileURLToPath(
            new URL("src/content/blog/", config.root),
          );
          ...
          for (const name of readdirSync(blogDirectory)) {
            ...
            if (!latest || lastmod > latest)
              latestPostByLanguage.set(data.lang, lastmod);
          ...
          for (const pair of translations.values()) {
            if (pair.es && pair.en) {
              addAlternates(
                `${routes.blog.es}${pair.es}/`,
                `${routes.blog.en}${pair.en}/`,
              );
            }
          }
        },
    ...
    serialize(item) {
      ...
      const lastmod =
        postLastmod ??
        (hubLanguage ? latestPostByLanguage.get(hubLanguage) : undefined);
