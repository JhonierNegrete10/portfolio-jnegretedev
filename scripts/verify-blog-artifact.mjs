...
function postArtifacts() {
  ...
  for (const root of roots) {
    ...
    for (const entry of readdirSync(root.directory, { withFileTypes: true })) {
      ...
      if (existsSync(htmlPath)) posts.push({ url: `${root.prefix}${entry.name}/`, htmlPath });
    }
  }
  return posts;
}
...
function verifyPost({ url, htmlPath }) {
  ...
  if (postingScripts.length !== 1) {
    ...
    try {
      ...
      fail(url, 'valid BlogPosting JSON-LD', error instanceof Error ? error.message : String(error));
    }
  }
...
function translatedPairs() {
  ...
  for (const name of readdirSync(contentDirectory)) {
    ...
    const { data } = matter(readFileSync(path.join(contentDirectory, name), 'utf8'));
...
const postLanguages = new Set(posts.map(({ url }) => (url.startsWith('/en/blog/') ? 'en' : 'es')));
